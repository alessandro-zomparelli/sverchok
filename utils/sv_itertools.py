...
# force verts option automatically convert 3 number arrays into tuples
def append_lists(l, lists, force_verts=False):
    if isinstance(l,(list)):
        flat_list = True
        for index, i in enumerate(l):
            if isinstance(i,(list)):
                ...
                if force_verts:
                    if len(i) == 3 and isinstance(i[0],(int,float)):
                        l[index] = tuple(i)
                    else:
                        flat_list = False
                        break
        ...
        if flat_list:
            ...
            return [append_lists(i, lists, force_verts) for i in l]
    else:
        lists.append([l])
        return None

# generate a single list with 1 level lists inside
def list_of_lists(l, force_verts=False):
    out_list = []
    append_lists(l, out_list, force_verts)
    return out_list

# works with irregular sublists
def match_longest_lists(lists):
    add_level = max([isinstance(l, list) for l in lists])
    # true if there is at least one list
    if add_level:
